...
"""Save the paths of crops accurated from IMAGENET VID 2015 dataset in pickle format
...
class Config:
  ...
  dataset_dir = '/home/v-chaoqw/MYSFC-ORI/ILSVRC2015-curated/'
  save_dir = '/home/v-chaoqw/MYSFC-ORI/workspace/'

  # percentage of all videos for validation
  validation_ratio = 0.01

  # number of video_dirs within each tfrecord files, note if may not be equal
  # to the number of object examples, since each video may contain several obj.
  iterator_size = 150
...
class Dataset:
  ...
  def dataset_iterator(self, video_dirs):
    ...
    iter_size = self.config.iterator_size
    ...
    for iter_ in range(iter_num):
      ...
      for index in range(num_videos):
        print('Processing {}/{}...'.format(index, num_videos))
        ...
        for trackid in trackids:
          ...
          instance_image_paths = sorted(instance_image_paths,
...
def main():
  ...
  if not tf.gfile.IsDirectory(config.save_dir):
    logging.info('Creating training directory: %s', config.save_dir)
    tf.gfile.MakeDirs(config.save_dir)

  with open(os.path.join(config.save_dir, 'val_imdb.pickle'), 'wb') as f:
